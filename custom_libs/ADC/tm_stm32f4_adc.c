...
void TM_ADC_EnableTSensor(void) {
    ADC_TempSensorVrefintCmd(ENABLE);
}

void TM_ADC_DisableTSensor(void) {
    ADC_TempSensorVrefintCmd(DISABLE);
}

uint16_t TM_ADC_ReadIntTemp(ADC_TypeDef* ADCx) {
  float result;
  
  ADC_RegularChannelConfig(ADCx, ADC_Channel_TempSensor, 1, ADC_SampleTime_144Cycles);
  
  result = TM_ADC_Read(ADC1, ADC_Channel_TempSensor);
  result *= 3.3;  //3.3 V power
  result /= 0xfff; //12 bit resolution, 2^12=4096=0xfff
  result -= 0.760; // Subtract the reference voltage at 25°C
  result /= .0025; // Divide by slope 2.5mV
